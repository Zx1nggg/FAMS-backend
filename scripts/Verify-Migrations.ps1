...
if ($sql.Contains('auth_version') -or $sql.Contains('CREATE TABLE t_inspection_record') -or $sql.Contains('ADD COLUMN pending_phone')) { throw 'Legacy fixture reconstruction failed' }
...
& "$MySqlBin/mysql.exe" @clientArgs fams_migration_smoke --execute="ALTER TABLE sys_user ADD UNIQUE INDEX username(username); ALTER TABLE sys_registration_application ADD UNIQUE INDEX uk_username(username), MODIFY COLUMN real_name VARCHAR(50) NOT NULL"
if ($LASTEXITCODE -ne 0) { throw 'Legacy registration constraints could not be reconstructed' }
...
foreach ($migration in @('migration-20260906-auth-version.sql','migration-20260906-delete-batch.sql','migration-20260906-inspections.sql','migration-20260906-pond-harvest.sql','migration-20260907-pending-registration.sql','migration-20260907-registration-name-fields.sql')) {
...
$registrationShape = & "$MySqlBin/mysql.exe" @clientArgs --batch --skip-column-names fams_migration_smoke --execute="SELECT CONCAT((SELECT IS_NULLABLE FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='sys_registration_application' AND column_name='username'),'/',(SELECT IS_NULLABLE FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='sys_registration_application' AND column_name='real_name'),'/',(SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='sys_registration_application' AND index_name='uk_username'),'/',(SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='sys_user' AND index_name='username'))"
if ($LASTEXITCODE -ne 0 -or $registrationShape -ne 'NO/YES/0/0') { throw "Registration name migration has an unexpected shape: $registrationShape" }
Write-Output 'PASS six incremental MySQL migrations; soft-deleted history preserved; two active ponds allowed'
